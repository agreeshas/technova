...
<title>🚩💚 Relationship Flag Detector</title>
...
<div class="app">
  ...
  <div class="header">
    ...
    <p class="header-desc">
      Paste text, record a voice note, or upload an audio file —
  ...
  <div class="footer">
    Relationship Flag Detector — awareness tool only.<br>
...
// ════════════════════════════════════════
const SAMPLES = [
  `"Baby I'm sorry I got upset. I love you so much it drives me crazy. I support everything you do — but you should spend less time with Maya, she's a bad influence. I just want us to be closer. You're everything to me."`,
  `"Hey, I noticed you seemed quiet and wanted to check in — how are you? No pressure. I know I messed up last week and you were completely right. I want to do better. You don't have to make yourself small for me."`,
  `"You went out AGAIN? If you really loved me you'd WANT to be with me. I gave up so much for this relationship. Go have fun — don't worry about me sitting here alone. I don't get why you need friends when you have me."`,
];
...
function renderSetup() {
  ...
  <div class="setup-card">
    <h3>⚙️ Quick Setup — Get Your Free Groq API Key</h3>
    <div class="setup-step">
      <div class="step-num">1</div>
      <div class="step-text">Go to <a href="https://console.groq.com" target="_blank">console.groq.com</a> → Sign up free (Google/GitHub)</div>
    </div>
    <div class="setup-step">
      <div class="step-num">2</div>
      <div class="step-text">Click <strong>"API Keys"</strong> in sidebar → <strong>"Create API Key"</strong> → Copy it</div>
    </div>
    <div class="setup-step">
    ...
    <div class="key-row">
      ...
      <button class="btn-analyze ready" style="width:auto;padding:12px 22px;margin:0;font-size:12px" onclick="saveKey()">Save →</button>
    </div>
    <p style="font-size:11px;color:var(--muted);margin-top:14px;line-height:1.7">
      🔒 Stored only in your browser — we never see it. Free tier: 14,400 requests/day.
    </p>
  </div>`;
}
...
function renderAnalyze() {
  const inputTabs = [
    { id:'text',   icon:'📝', label:'Text',       sub:'Paste message',    cls:'tab-text'   },
    { id:'record', icon:'🎤', label:'Record',      sub:'Use microphone',   cls:'tab-record' },
    { id:'upload', icon:'📁', label:'Upload',      sub:'Audio file',       cls:'tab-upload' },
  ];
  const modes = [
    { id:'both',  label:'🚩💚 Both',     cls: state.analyzeMode==='both'  ? 'active-both'  : '' },
    { id:'red',   label:'🚩 Red Only',   cls: state.analyzeMode==='red'   ? 'active-red'   : '' },
    { id:'green', label:'💚 Green Only', cls: state.analyzeMode==='green' ? 'active-green' : '' },
  ];
  ...
  <!-- ERROR -->
  ${state.error ? `<div class="error-box">⚠️ ${esc(state.error)}</div>` : ''}

  <!-- ANALYZE BUTTON -->
  <button class="btn-analyze ${canAnalyze ? 'ready' : ''}" onclick="startAnalysis()">
    🔍 Analyze Relationship Flags
  </button>

  <p class="privacy-note">🔒 Private & anonymous — no data stored anywhere.</p>

  <div style="text-align:right">
    <button class="btn-small" onclick="goSetup()">⚙️ Change API Key</button>
  </div>`;
}
...
function renderTextPanel() {
  ...
  <div class="card">
    ...
    <div class="examples">
      ...
      <button class="ex-btn" onclick="clearText()">Clear ✕</button>
    </div>
  </div>`;
}
...
function renderRecordPanel() {
  ...
  return `
  <div class="card">
    <div class="card-label">🎤 Record Voice Note or Conversation</div>
...
function renderUploadPanel() {
  return `
  <div class="card">
    <div class="card-label">📁 Upload Voice Note or Call Recording</div>
    ...
    ${!state.audioFile ? `
      <div class="upload-zone" id="uploadZone"
        ...
        <p class="upload-formats">MP3 • WAV • M4A • OGG • WEBM • FLAC</p>
      </div>
    ` : `
    ...
    <p style="font-size:11px;color:var(--muted);margin-top:14px;line-height:1.7">
      💡 Works with WhatsApp voice notes, call recordings, video files (audio extracted).<br>
...
function renderLoading() {
  ...
  <div class="loading-wrap">
    <div class="spin-emoji">${isAudio ? '🎧' : '🔍'}</div>
...
function renderResult() {
  ...
  const gradeEmojis = {TOXIC:'💔',CONCERNING:'⚠️',MIXED:'⚖️',GOOD:'💛',EXCELLENT:'💚'};
  const color = gradeColors[grade] || '#f6e05e';
  const emoji = gradeEmojis[grade] || '⚖️';
  ...
  if (state.transcript) {
    ...
    <div class="transcript-box">
      <div class="card-label">🎙️ Transcription (what the AI heard)</div>
  ...
  <div class="health-banner">
    ...
    <div class="meter-labels">
      <span class="meter-label red">🚩 ${r.red_flag_count||0} Red Flag${(r.red_flag_count||0)!==1?'s':''}</span>
      <span class="meter-label green">${r.green_flag_count||0} Green Flag${(r.green_flag_count||0)!==1?'s':''} 💚</span>
    </div>
  ...
  html += `<div class="flags-grid">
    <div class="flags-col col-red">
      <div class="col-head"><span class="col-icon">🚩</span><span class="col-title">Red Flags</span></div>
    ...
    <div class="flags-col col-green">
      <div class="col-head"><span class="col-icon">💚</span><span class="col-title">Green Flags</span></div>
  ...
  if(r.you_deserve){html+=`<div class="affirm-card"><div class="card-label">💛 Remember This</div><p>${esc(r.you_deserve)}</p></div>`;}

  if(r.what_to_watch&&r.what_to_watch.length>0){
    html+=`<div class="actions-card"><div class="card-label">💡 What To Pay Attention To</div>${r.what_to_watch.map(i=>`<div class="action-item"><span class="action-arrow">→</span><span class="action-text">${esc(i)}</span></div>`).join('')}</div>`;
  }

  if(['TOXIC','CONCERNING','MIXED'].includes(grade)&&r.resources){
    html+=`<div class="resources-card"><div class="card-label">🆘 Support Resources</div>${r.resources.map(r=>`<div class="resource-line">${esc(r)}</div>`).join('')}</div>`;
  }

  html+=`<button class="btn-small" onclick="resetApp()">← Analyze Another</button>`;
...
async function startAnalysis() {
  ...
  try {
    ...
    if (!textToAnalyze || textToAnalyze.length < 5) {
      throw new Error('Could not transcribe audio — the recording may be too short or silent.');
    }
...
2. Click the red mic button → allow microphone access
3. Speak the conversation or describe the situation
4. Click the button again to stop
5. You'll see a playback bar — listen back to confirm
6. Click "Analyze Relationship Flags"
7. AI transcribes it first → then detects flags!
```
...
4. Works with WhatsApp voice notes!
   (Go to WhatsApp → hold voice note → Export → save the file)
